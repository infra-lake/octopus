...
export async function get({ data }) {
    ...
    const response = await http({
        request: {
            url: url({ path: '/api/v1/roles', query: { limit: 1000000 } }),
    ...
    const result = list.roles
        .filter(role => key({ data: role }) === key({ data }))
...
export async function put({ data }) {
    ...
    const response = await http({
        request: {
            url: url({ path: `/api/v1/roles/${ key({ data }) }` }),
